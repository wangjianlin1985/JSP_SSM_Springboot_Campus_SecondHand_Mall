...
public class OrderInfoController extends BaseController {
	...
	/*前台按照查询条件分页查询订单信息*/
	@RequestMapping(value = { "/sellerFrontlist" }, method = {RequestMethod.GET,RequestMethod.POST})
	public String sellerrontlist(String orderNo,@ModelAttribute("userObj") UserInfo userObj,String orderStateObj,String orderTime,String receiveName,String telephone,@ModelAttribute("sellObj") UserInfo sellObj,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (orderNo == null) orderNo = "";
		if (orderStateObj == null) orderStateObj = "";
		if (orderTime == null) orderTime = "";
		if (receiveName == null) receiveName = "";
		if (telephone == null) telephone = "";
		sellObj = new UserInfo();
		sellObj.setUser_name(session.getAttribute("user_name").toString());
		List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(orderNo, userObj, orderStateObj, orderTime, receiveName, telephone, sellObj, currentPage);
	    /*计算总的页数和总的记录数*/
	    orderInfoService.queryTotalPageAndRecordNumber(orderNo, userObj, orderStateObj, orderTime, receiveName, telephone, sellObj);
	    /*获取到总的页码数目*/
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "OrderInfo/orderInfo_sellerFrontquery_result"; 
	}
