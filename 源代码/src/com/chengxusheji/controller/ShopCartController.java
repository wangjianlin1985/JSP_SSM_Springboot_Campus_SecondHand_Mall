...
public class ShopCartController extends BaseController {
	...
	/*前台用户客户端ajax方式提交添加购物车信息*/
	@RequestMapping(value = "/userAdd", method = RequestMethod.POST)
	public void userAdd(ShopCart shopCart,
		...
		Product product = productService.getProduct(shopCart.getProductObj().getProductId());
		if(product.getUserObj().getUser_name().equals(userName)) {
			message = "你不能购买自己的商品";
			writeJsonResponse(response, success, message);
			return ;	
		}
		
		ArrayList<ShopCart> cartList = shopCartService.queryShopCart(shopCart.getProductObj(), userObj);
		if(cartList.size() > 0) {
	...
	/*前台用户按照查询条件分页查询自己的购物车信息*/
	@RequestMapping(value = { "/userFrontlist" }, method = {RequestMethod.GET,RequestMethod.POST})
	public String userFrontlist(@ModelAttribute("productObj") Product productObj,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1; 
		String userName = (String) session.getAttribute("user_name"); 
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "ShopCart/shopCart_userFrontquery_result"; 
	}
