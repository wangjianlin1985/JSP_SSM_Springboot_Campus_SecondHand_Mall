...
public class ProductController extends BaseController {
	...
	/*用户前台按照查询条件分页查询自己发布的商品信息*/
	@RequestMapping(value = { "/userFrontlist" }, method = {RequestMethod.GET,RequestMethod.POST})
	public String userFrontlist(@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,@ModelAttribute("userObj") UserInfo userObj,String addTime,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (productName == null) productName = "";
		if (addTime == null) addTime = "";
	...
	public String frontshow(@PathVariable Integer productId,Model model,HttpServletRequest request) throws Exception {
        ...
        ArrayList<Comment> commentList = commentService.queryComment(product, null, "");
        List<ProductClass> productClassList = productClassService.queryAllProductClass();
        request.setAttribute("productClassList", productClassList);
        List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
